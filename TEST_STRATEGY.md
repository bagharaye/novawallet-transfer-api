...
1. **Concurrency beyond what's covered** — concurrent idempotency-key replay races (covered) and concurrent transfers racing the daily limit (also covered, and confirmed the per-wallet lock design already prevents the TOCTOU race — see `ConcurrencyTests.ConcurrentTransfers_EachUnderDailyLimitButCollectivelyOverIt_NeverExceedTheLimit`) turned out cheap enough to actually add rather than leave as a gap. What's *still* not covered: concurrent credit + transfer interleavings on the same wallet, and concurrency across a much larger number of distinct wallets than the handful used in these tests (lock-table growth, contention patterns at scale).
...
3. **Deeper security** — timing-based token comparison, rate limiting/brute-force on the token, and a wider injection/fuzz corpus (property-based testing on amount and ID fields).
4. **Load/performance beyond the smoke test done here** — the stretch-goal k6 run (`load/`) confirmed no 5xx and low latency under a short, moderate-concurrency burst against a small wallet pool; it did not cover sustained duration (memory/GC pressure over minutes-to-hours), a realistic wallet-count scale, or deliberately concentrated ("hot wallet") contention — see `load/README.md`'s findings for the full caveat.
5. **Long-running/scheduled behavior** — the WAT-reset test proves correctness *at* the boundary; it doesn't run for real multi-day soak time to catch drift (leap seconds, DST-adjacent host misconfiguration, clock skew between app and DB).
...
With a week, priority order would follow the risk table in §1: fault injection first (highest remaining financial blast radius and the least covered by the time-boxed suite), then the deeper security pass, then load testing at realistic scale and duration.
